...
def process_frame(frame, tracker):
    ...
    saved_file = None
    if capture_needed and tracker.can_capture():
        saved_file = save_capture(frame, capture_reason)
        tracker.mark_captured()
        if left_right_count >= 3:
            tracker.left_right_count = 0

    return frame, score, reasons, status, saved_file
...
if mode == "Image":
    ...
    if uploaded_file is not None:
        ...
        nparr = bytearray(file_bytes)
        frame = cv2.imdecode(
            cv2.UMat(bytearray(nparr)).get() if False else cv2.imdecode.__self__,
            1
        )